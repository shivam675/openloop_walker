...
class RexGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if done:
            self.rex.Terminate()
        # print(np.array(self._get_observation()))
    ...
    def _reward(self):
        ...
        # check = True if current_x >= self._target_position + 0.2 else False
        # if check:
        #     if self.body_x_vel <= 0.0001:
        #         self.env_goal_reached = True

        # forward_reward = self.body_x_vel
